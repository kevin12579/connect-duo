...
/**
 * [내부 공통 함수] 세무사 통계(TaxStats) + 응답속도 업데이트
...
async function updateTaxStatsInternal(tax_id) {
    ...
    // 2. 세무사의 user_id 조회 (ConsultRooms 조회용)
...
exports.usercomment = async (req, res) => {
    ...
    try {
        ...
        res.json({
            result: 'success',
            data: { user: { ...users[0], bio_one_line: bio }, comments },
        });
    } catch (error) {
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. 세무사 상세 정보 조회 (response_speed 포함)
// ─────────────────────────────────────────────────────────────────────────────
exports.taxpro = async (req, res) => {
    ...
    try {
        // 세무사 프로필 + response_speed 조회
        const taxProSql = `
            SELECT tap.id, tap.user_id, u.name, u.profile_img,
                   tap.bio_one_line, tap.response_speed,
        ...
        const taxProProfileId = taxPros[0].id;

        // 통계 조회
        const [stats] = await pool.query(
            `SELECT recommend_count, satisfaction_score, re_consult_rate, consult_count
             FROM TaxStats WHERE tax_id = ?`,
            [taxProProfileId],
        );
        ...
        if (viewerId) {
            const [roomRows] = await pool.query(
                `SELECT id FROM ConsultRooms
                 WHERE user_id = ? AND tax_id = ? AND status = 'ACTIVE'`,
            ...
            if (roomRows.length > 0) {
                ...
                const [reqRows] = await pool.query(
                    `SELECT status FROM ConsultRequests
                     WHERE user_id = ? AND tax_user_id = ?`,
                    [viewerId, taxProUserId],
                );
        ...
        res.json({
            result: 'success',
            data: {
                taxPro: taxPros[0], // response_speed 포함
                stats: stats[0] || {},
                comments,
                requests,
                consultStatus,
